...
import { useServerProgress } from '@/lib/hooks/use-server-progress'
...
export function DashboardPhaseAccordion({
  phases,
  onProgressChange,
}: {
  phases: Phase[]
  ...
  const { toggleTask, isComplete, completedCount, loading } = useServerProgress()
  ...
  async function handleToggle(taskId: string) {
    ...
    if (result.completed && result.quote) setQuote(result.quote)
    onProgressChange?.(completedCount, totalTasks)
  }
    ...
    <div>
      <div className="mb-8">
        <div className="flex items-center justify-between text-sm text-text-muted mb-2">
          <span>{loading ? 'Loading progress...' : `${completedCount} of ${totalTasks} tasks complete`}</span>
